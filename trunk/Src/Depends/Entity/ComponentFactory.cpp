...
ComponentFactory::ComponentFactory(Core::CoreManager *coreMgr)
{
...
Component* ComponentFactory::CreateComponent(Scene::Object *go, const CL_String& compType, const CL_String& compName)
{
	if(creators == 0)
		return 0;
	...
	if(creatorIt == creators->end())
		throw T_Exception("Unable to create component " + compType);
	
	ComponentCreator creator = creatorIt->second;

	if(compName.empty())
		return creator(coreMgr, go, compType);
	else
		return creator(coreMgr, go, compName);
}
